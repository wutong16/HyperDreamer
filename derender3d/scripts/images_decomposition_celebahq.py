...
test_path_extracted = Path('datasets') / 'celebahq' / 'extracted' / 'test'

out = Path('results') / 'images' / 'decomposition' / 'celebahq'
...
indices = first_figure
# indices = indices[6:8]
# indices = [331, 355, 357, 400, 452, 470]
...
def main():
    ...
    dataset = ImageDataset(str(test_path), image_size=image_size, crop=None, is_validation=True, precomputed_dir=test_path_precompute, extracted_dir=test_path_extracted, cfgs={'min_depth': .9, 'max_depth': 1.1})
    ...
    # models = [model_classical, model_nr, model_shading]
    # cps = [cp_classical, cp_nr, cp_shading]
