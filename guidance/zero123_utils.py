...
class Zero123(nn.Module):
    def __init__(self, device, fp16, vram_O=False, t_range=[0.02, 0.98],
                 ...
                 ckpt = './pretrained/zero123/zero123-xl.ckpt', opt=None):
        super().__init__()
    ...
    def _annealed_sampling(self, global_step):
        t_min, t_max = self.min_step, self.max_step
        t = t_max - (t_max - t_min) * 1. * np.sqrt(global_step / self.opt.iters)
        return int(t)
    ...
    def train_step(self, embeddings, pred_rgb, polar, azimuth, radius, guidance_scale=3, as_latent=False, grad_scale=1, save_guidance_path:Path=None, global_step:int=None):
        ...
        with torch.no_grad():
            ...
            for (zero123_w, c_crossattn, c_concat, ref_polar, ref_azimuth, ref_radius) in zip(zero123_ws.T,
                ...
                if self.opt.stable123:
                    T = torch.stack([torch.deg2rad(p), torch.sin(torch.deg2rad(-a)), torch.cos(torch.deg2rad(a)), torch.deg2rad(torch.full_like(p, ref_polar))], dim=-1)[:, None, :] # XXX only one ref image
                else:
        ...
        w = (1 - self.alphas[t]) * (self.alphas[t]**0.5) # fantasia3d weight setting
