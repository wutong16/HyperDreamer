...
def prepend_dims(tensor, shape):
    ...
    tensor = tensor.view([1] * len(shape) + orig_shape).expand(
        shape + [-1] * len(orig_shape))
    return tensor
...
def hemisphere_int(lambda_val, cos_beta):
    ...
    A_b = 2. * np.pi / lambda_val * (
                torch.exp(-lambda_val) - torch.exp(-2. * lambda_val))
...
def render_with_sg(lgtSGs, specular_reflectance, roughness, diffuse_albedo,
    ...
    :param diffuse_a lbedo: [..., 3]; values must lie in [0,1]
    ...
    dots_shape = list(normal.shape[:-1])  # the number of points or normals
    ...
    lgtSGs = lgtSGs.unsqueeze(-2).expand(dots_shape + [M, K, 7]).clone()  # [..., M, K, 7]
    # lgtSGs[:,:,:,4]=0
    # lgtSGs[:,:,:,5]=0
    # lgtSGs[:,:,:,6]=1.2
    ...
    brdfSGLambdas = (2. * inv_roughness_pow4).unsqueeze(-2).unsqueeze(
        -2).expand(dots_shape + [M, K, 1])
    # mu_val = (inv_roughness_pow4 / np.pi).expand([K, 3])  # [K, 1] ---> [K, 3]
    mu_val = (inv_roughness_pow4 / np.pi).expand(dots_shape + [3])  # [K, 1] ---> [K, 3]
    # brdfSGMus = prepend_dims(mu_val, dots_shape + [M, ])  # [..., M, K, 3]
    brdfSGMus = mu_val.unsqueeze(-2).unsqueeze(-2).expand(dots_shape + [M, K, 3])
    ...
    k = ((roughness + 1.) * (roughness + 1.) / 8.).unsqueeze(-2).unsqueeze(
        -2).expand(dots_shape + [M, K, 1])
    ...
    final_lobes, final_lambdas, final_mus = lambda_trick(lgtSGLobes,
                                                         lgtSGLambdas, lgtSGMus,
                                                         warpBrdfSGLobes,
                                                         warpBrdfSGLambdas,
                                                         warpBrdfSGMus)
    ...
    lambda_cos = 0.0315
    alpha_cos = 31.7003
    lobe_prime, lambda_prime, mu_prime = lambda_trick(normal, lambda_cos,
                                                      mu_cos,
                                                      final_lobes,
                                                      final_lambdas, final_mus)

    dot1 = torch.sum(lobe_prime * normal, dim=-1, keepdim=True)
    dot2 = torch.sum(final_lobes * normal, dim=-1, keepdim=True)
    # [..., M, K, 3]
    specular_rgb = mu_prime * hemisphere_int(lambda_prime,
                                             dot1) - final_mus * alpha_cos * hemisphere_int(
        final_lambdas, dot2)
    ...
    if diffuse_rgb is None:
        ...
        diffuse_rgb = mu_prime * hemisphere_int(lambda_prime, dot1) - \
                      final_mus * alpha_cos * hemisphere_int(final_lambdas,
                                                             dot2)
