...
class _fresnel_shlick_func(torch.autograd.Function):
    ...
    def backward(ctx, dout):
        f0, f90, cosTheta = ctx.saved_tensors
...
class _ndf_ggx_func(torch.autograd.Function):
    ...
    @staticmethod
    def backward(ctx, dout):
        alphaSqr, cosTheta = ctx.saved_tensors
...
class _masking_smith_func(torch.autograd.Function):
    ...
    @staticmethod
    def backward(ctx, dout):
        alphaSqr, cosThetaI, cosThetaO = ctx.saved_tensors
...
class _prepare_shading_normal_func(torch.autograd.Function):
    ...
    @staticmethod
    def backward(ctx, dout):
        pos, view_pos, perturbed_nrm, smooth_nrm, smooth_tng, geom_nrm = ctx.saved_tensors
...
class _frostbite_diffuse_func(torch.autograd.Function):
    ...
    @staticmethod
    def backward(ctx, dout):
        nrm, wi, wo, linearRoughness = ctx.saved_tensors
...
class _pbr_specular_func(torch.autograd.Function):
    ...
    @staticmethod
    def backward(ctx, dout):
        col, nrm, wo, wi, alpha = ctx.saved_tensors
