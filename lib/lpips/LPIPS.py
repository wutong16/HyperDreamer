...
class LPIPS(nn.Module):
    def __init__(self, pretrained=True, net='vgg', version='0.1', lpips=False,
                 spatial=False,
                 pnet_rand=False, pnet_tune=False, use_dropout=True,
            ...
            if (pretrained):
                if (model_path is None):
                    ...
                    model_path = os.path.abspath(
                        os.path.join(inspect.getfile(self.__init__), '../..',
...
class L2(FakeNet):
    def forward(self, in0, in1, retPerLayer=None):
        ...
        if (self.colorspace == 'RGB'):
            ...
            value = lpips.l2(lpips.tensor2np(
                lpips.tensor2tensorlab(in0.data, to_norm=False)),
                             lpips.tensor2np(lpips.tensor2tensorlab(in1.data,
                                                                    to_norm=False)),
...
class DSSIM(FakeNet):
    ...
    def forward(self, in0, in1, retPerLayer=None):
        ...
        if (self.colorspace == 'RGB'):
            value = lpips.dssim(1. * lpips.tensor2im(in0.data),
                                1. * lpips.tensor2im(in1.data),
                                range=255.).astype('float')
        elif (self.colorspace == 'Lab'):
            value = lpips.dssim(lpips.tensor2np(
                lpips.tensor2tensorlab(in0.data, to_norm=False)),
                                lpips.tensor2np(lpips.tensor2tensorlab(in1.data,
                                                                       to_norm=False)),
                                range=100.).astype('float')
...
if __name__ == "__main__":
    ...
    img1 = lpips.im2tensor(lpips.load_image(args.img1))
    img2 = lpips.im2tensor(lpips.load_image(args.img2))
