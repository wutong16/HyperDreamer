...
def adam(params: List[Tensor],
    ...
    for i, param in enumerate(params):
        ...
        if amsgrad:
            ...
            denom = (max_exp_avg_sqs[i].sqrt() / math.sqrt(
                bias_correction2)).add_(eps)
        else:
...
def load_checkpoint(model, optimizer, ckpt_path, no_reload_optimizer,
    ...
    if model.rgbnet[0].weight.shape != ckpt['model_state_dict'][
        'rgbnet.0.weight'].shape:
...
def load_grid_data(model, ckpt_path, deduce=1, name='density',
                   return_raw=False):
    ckpt = torch.load(ckpt_path)
...
def load_weight_by_name(model, ckpt_path, deduce=1, name='density',
                        return_raw=False):
    ckpt = torch.load(ckpt_path)
...
def load_model(model_class, ckpt_path, new_kwargs=None, strict=False):
        ...
        for k, v in new_kwargs.items():
            if k in ckpt['model_kwargs']:
                if ckpt['model_kwargs'][k] != v:
                    print('updating {} from {} to {}'.format(k, ckpt[
                        'model_kwargs'][k], v))
    ...
    try:
        ...
        if strict:
            ...
            print(
                ">>> Checkpoint loaded without strict matching from {}".format(
                    ckpt_path))
    return model
...
def analyze_point_cloud(filename=None, log_num=18, rand_offset=False,
    ...
    for i in range(int(np.ceil(len(points) / batch_size))):
        pts = torch.from_numpy(
            point_[i * batch_size: (i + 1) * batch_size]).cuda()
        sdf = -query_func(pts)
        sdfs.append(sdf.cpu().numpy())
    sdfs = np.hstack(sdfs)
    colors = (color_map_color(sdfs * 0.5 + 0.5) * 255).astype(np.uint8)

    vertexs = np.array([tuple(v) for v in points],
                       dtype=[('x', 'f4'), ('y', 'f4'), ('z', 'f4')])
    vertex_colors = np.array([tuple(v) for v in colors],
    ...
    print(">>> Points saved at {}".format(
        os.path.join(save_root, "gt_pcd_eval.ply")))
    return


def load_point_cloud(filename=None, log_num=17, rand_offset=False,
                     load_normal=False, save_root=''):
    plydata = PlyData.read(filename)
    ...
    print(">>> Points saved at {}".format(
        os.path.join(save_root, "est_normal.ply")))
    exit()
    return
...
def write_ply(points, filename, colors=None, normals=None):
    ...
    if normals is not None:
        vertex_normal = np.array([tuple(n) for n in normals],
                                 dtype=[('nx', 'f4'), ('ny', 'f4'),
                                        ('nz', 'f4')])
...
class GradLayer(nn.Module):
    ...
    def __init__(self, ksize=3):
        ...
        kernel_v = torch.FloatTensor(
            kernel_v / np.abs(kernel_v).sum()).unsqueeze(0).unsqueeze(0)
        kernel_h = torch.FloatTensor(
            kernel_h / np.abs(kernel_h).sum()).unsqueeze(0).unsqueeze(0)
...
class GaussianLayer(nn.Module):
    ...
    def __init__(self, ksize=3):
        ...
        kernel = torch.FloatTensor(kernel / np.abs(kernel).sum()).unsqueeze(
            0).unsqueeze(0)
...
def _gaussian_3dconv(ksize=3, sigma=1):
    ...
    m = nn.Conv3d(1, 1, ksize, stride=1, padding=ksize // 2,
...
class GradLoss(nn.Module):
    ...
    def forward(self, output, gt_img, savedir=''):
        ...
        if savedir:
            img1 = np.concatenate(
                [to8b(gt_img.detach().cpu().numpy())[0, 0][..., None],
                 to8b(5 * gt_grad.detach().cpu().numpy())[0, 0][..., None],
                 to8b(5 * gt_grad.detach().cpu().numpy())[0, 1][..., None]],
                axis=1)
            img2 = np.concatenate(
                [to8b(output.detach().cpu().numpy())[0, 0][..., None],
                 to8b(5 * output_grad.detach().cpu().numpy())[0, 0][..., None],
                 to8b(5 * output_grad.detach().cpu().numpy())[0, 1][..., None]],
                axis=1)
            ...
            imageio.imwrite(os.path.join(savedir, "debug_figs",
                                         "grad_module_{}.png".format(loss)),
                            img8)
        return loss


def rgb_to_luminance(rgb, return_chromaticity=False, gamma_correction=False,
                     lum_avg=1):
...
def get_sobel(img, ksize=3, thrd=0.1, g_ksize=0, d_ksize=0, suffix='',
              vis=False):
    if img.shape[-1] > 1:
    ...
    if vis:
        ...
        plt.savefig("debug_figs/test_sobel_k{}_{}.jpg".format(ksize, suffix),
                    dpi=200)
    return gaussian
...
def up_sample(rays_o, rays_d, z_vals, sdf, n_importance, inv_s):
    ...
    # |
    # ----------------------------------------------------------------------------------------------------------
    prev_cos_val = torch.cat([torch.zeros([batch_size, 1]), cos_val[:, :-1]],
                             dim=-1)
    ...
    weights = alpha * torch.cumprod(
        torch.cat([torch.ones([batch_size, 1]), 1. - alpha + 1e-7], -1), -1)[:,
                      :-1]
...
def sample_pdf(bins, weights, n_samples, det=False):
    ...
    if det:
        u = torch.linspace(0. + 0.5 / n_samples, 1. - 0.5 / n_samples,
                           steps=n_samples)
...
def sph_harm_coeff(l, m, k):
    ...
    return (np.sqrt(
        ...
        (4.0 * np.pi * np.math.factorial(l + m))) * assoc_legendre_coeff(l, m,
                                                                         k))
...
@torch.no_grad()
def get_surface_sliding(sdf, resolution=512, grid_boundary=[-1.1, 1.1],
                        level=0):
            ...
            for k in range(N):
                ...
                def evaluate(points):
                    ...
                    for _, pnts in enumerate(
                            torch.split(points, 100000, dim=0)):
                        z.append(sdf(pnts))
                    z = torch.cat(z, axis=0)
                    return z

                # construct point pyramids
                points = points.reshape(cropN, cropN, cropN, 3).permute(3, 0, 1,
                                                                        2)
...
class ScaleAndShiftInvariantLoss(nn.Module):
    def __init__(self, alpha=0.5, scales=4, reduction='batch-based',
                 ema_scale_shift=False, momentum=0.9, detach_scale_shift=False):
        super().__init__()
    ...
    def forward(self, prediction, target, mask, share_scale_shift=False):
        ...
        if share_scale_shift:
            ...
            scale_, shift_ = compute_scale_and_shift(prediction_, target_,
                                                     mask_)
            ...
            if self.ema_scale_shift:
                ...
                self.scale.data = self.momentum * self.scale.data + (
                            1 - self.momentum) * scale_
                self.shift.data = self.momentum * self.shift.data + (
                            1 - self.momentum) * shift_
