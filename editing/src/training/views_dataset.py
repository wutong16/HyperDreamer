...
from editing.src.configs.train_config import RenderConfig
...
def rand_poses(size, device, radius_range=(1, 1.6), theta_range=(0, 150), phi_range=(0, 360),
...
class MultiviewDataset:
    def __init__(self, cfg: RenderConfig, device, alternate_views=True):
        super().__init__()

        self.cfg = cfg
        self.device = device
        self.type = type  # train, val, tests
        size = self.cfg.n_views # Number of views to use. default=8
        
        self.phis = [(index / size) * 360 for index in range(size)]
        self.thetas = [self.cfg.base_theta for _ in range(size)] # Theta value for rendering during training

        # Alternate lists:filp views to symmetry
        ...
        logger.info(f'Init_train_dataloader —— phis: {self.phis}')
        ...
        logger.info(f'train_dataloader —— phis  : {self.phis}')
        logger.info(f'train_dataloader —— thetas: {self.thetas}')
        logger.info(f'train_dataloader —— size  : {self.size}')
...
class ViewsDataset:
    def __init__(self, cfg: RenderConfig, device, size=100,type='circle'):
        super().__init__()

        self.cfg = cfg
        self.device = device
        self.type = type  # circle,rand
        self.size = size
        
        logger.info(f'val_dataloader —— size: {self.size}')
    ...
    def collate(self, index):
        ...
        if self.type =='circle':
            ...
            dirs, thetas, phis, radius = rand_poses(B, self.device, radius_range=self.cfg.radius_range,
                                        angle_overhead=self.cfg.overhead_range, angle_front=self.cfg.front_range)
